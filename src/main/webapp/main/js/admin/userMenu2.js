...
function logList_ajax(i){
   ...
   //ajax 전송
   $.ajax({
      url : '/admin/ajax/authMenagementList.do',
      data : data,
      dataType : 'json',
      ...
      success : function(data) {
         ...
         for(var idx in data.accessLogList){
            var ListHTML='<tr class="odd gradeX" onclick="selectMemberInfo('+ accessLogList[idx].id + ')">';
            var contHTML='<td>'+accessLogList[idx].uNm+'</td>';
            contHTML+='<td>'+accessLogList[idx].uHp+'</td>';
            contHTML+='<td>'+accessLogList[idx].cNm+'</td>';
            contHTML+='<td><span class="badge bg-warning">certified</span></td>';
