...
function init(){
	...
	$(".closebtn").on("click", function(){  // 사이드 팝업창 close 버튼
		...
		if(confirmedEmailValue != ""){
			$('#join_id').val(confirmedEmailValue);
			if(confirmedEmail == true){
				$('#mail_check_input').val(code);
			}
		}
...
/*	$("#campaign_joinform").keydown(function(key) {
	...
	$("#member_pwChangeForm").validate({
	    ...
	    submitHandler: function(form) {
	    	...
	    	$.ajax({ //이전 동일 비밀번호 체크
				url:"/member/change/passwordSameCk.do",
				data: data,
				dataType : 'json', 
				async: false,
				type:"GET",
	    	...
	    	if(CryptoJS.SHA256($('#pwchkChange_pw').val()).toString() == beforePW){
	    		...
	    		$.ajax({ //비밀번호 변경ajax
					url:"/member/change/password.do",
					data:{uId : CryptoJS.SHA256($("#pwChange_id").val()).toString(),
						  uPw : CryptoJS.SHA256($("#pwChange_pw").val()).toString()},
					type:"get",
					success:function(data){
						localStorage.removeItem("rememberPw");
	...
	if (!regExp.test(email)) {
		...
		if(emailConfirm(email)==true){
			if(confirm("사용 가능한 이메일입니다. 확인을 누르시면 입력하신 이메일을 통해 이메일 인증이 진행됩니다.")){
				//이메일 인증 진행
				if(isMailSend == false && emailCertification(email)==true){
					alert("메일이 발송되었습니다. 입력하신 이메일 주소에서 발급된 코드를 확인하세요.");
					isMailSend = true;
				}else{
					alert("이메일 인증을 이미 요청하였습니다.");
				}
			}else{
...
function emailCertification(email){
	...
	$.ajax({
		url: "/member/ajax/sendJoinCode.do",
		type : "post",		
		async:false,
		data : data,
		...
		success:function(data){
			if(data){
				//이메일 발송 성공
				flag=true;
				confirmedEmailValue = $("#join_id").val();
...
(function() {
    document.addEventListener('scroll', function() {
        var currentScrollValue = document.documentElement.scrollTop;
          if(Math.floor(currentScrollValue)>2968){
            $('li').removeClass('active');
            $('.index-slide2').addClass('active');
          }
          if(Math.floor(currentScrollValue)>5058){
            $('li').removeClass('active');
            $('.index-slide3').addClass('active');
          }
          if(Math.floor(currentScrollValue)<2838){
            $('li.index-slide2').removeClass('active');
            $('.index-slide').addClass('active');
          }
      });   
})();