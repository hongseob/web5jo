...
<!-- Demo styles -->
<link rel="stylesheet" href="/css/egovframework/main/modify/campaignModify.css">
					...
					<div class="page2">
						<div class="page2_sub">
							...
							<div class="cam2_bottom">
								<div class="outBox1">
									<div class="inputBox">
										<input type="text" id="campainSetName" value="${campaignInfo[0].cNm}"
											class="textbox"> <label for="sampleId">캠페인
											이름을 입력하세요</label>
							...
							<div class="cam3_bottom">
								...
								<div class="cam3_imgbox">
									<img class="pre_pic" id="pre_pic" src="/imgpath/${campaignImg[0].fileSvrName }" onerror="this.src='/images/egovframework/main/common/noImage.png';"></img>
								...
								<div id="img_exist" style="position: absolute; right: 115px; top: 684px; width: 150px; background-color: white; text-align: left">
									${campaignImg[0].fileOrgName}
								</div>
							</div>
						</div>
					</div>
					...
					<div class="page3">
							...
							<div class="cam4_bottom">
								...
								<div class="campain_join">
									<input type="text" placeholder="0" id="campain_personnel"
										class="textboxSm" value="${campaignInfo[0].cMbr }">명
							...
							<div class="cam5_bottom">
								<div class="cam5_1">등록할 캠페인에 대한 참여 방식을 설정합니다.</div>
									<div class="campain_inter">
										<input type="radio" name="inter" value="대면" style="width:15px;height:15px;border:1px;accent-color:black;" <c:if test="${campaignInfo[0].cInter eq '1'}">checked</c:if>>&nbsp; 대면
									</div>
									<div class="campain_inter">
										<input type="radio" name="inter" value="비대면" style="width:15px;height:15px;border:1px;accent-color:black;" <c:if test="${campaignInfo[0].cInter eq '2'}">checked</c:if>>&nbsp; 비대면
									</div>
						...
						<!-- 		<div class="campain_pay1">
								...
								<div class="campain_pay2">
									<span value="총금액" style="width:15px;height:15px;border:1px;accent-color:black;">&nbsp; 총금액</span>
									<input type="text" placeholder="0" id="full_money" class="pay_textbox" value="${campaignInfo[0].estimate }"><span class="pay_textboxAfter">원 이상</span>
								</div>
								<div id="campain_pay_alert"></div>
							</div>
						</div>
					</div>
				</div>
						...
						<div class="page5_sub">
							...
							<div class="cam6_bottom_v2">
								...
								<div class="cam6_service">
									...
									<div id="select_box">
										<label for="color"><c:if test="${not empty campaignInfo[0].svcNm}">${campaignInfo[0].svcNm}</c:if><c:if test="${empty campaignInfo[0].svcNm}">ex) 줌, 디스코드</c:if></label> <select id="color"
											title="select color" >
											<option>ex) 줌, 디스코드</option>
											<option <c:if test="${campaignInfo[0].svcNm eq 'ZOOM'}">selected</c:if>>ZOOM</option>
											<option <c:if test="${campaignInfo[0].svcNm eq 'DISCORD'}">selected</c:if>>DISCORD</option>
										</select>
									...
									<div class="outBox3">
										<div class="inputBox">
											<input type="text" id="campainLinkLoc" class="textbox"
												name="" <c:if test="${not empty campaignInfo[0].svcLink}" >value="${campaignInfo[0].svcLink }"</c:if>> <label for="sampleId">링크를 입력하세요.</label>
											<button class="cam3_namebtn btn btn-dark"
							...
							<div class="cam7_bottom">
								...
								<div class="start_date">
									<label class="s_name">시작일</label> <input type="text"
										id="datePicker_start" class="datePicker" onclick='javascript:m_datepicker(this);' value="${campaignInfo[0].stDate }"> <span
								...
								<div class="end_date">
									<label class="e_name">종료일</label> <input type="text"
										id="datePicker_end" class="datePicker" onclick='javascript:m_datepicker(this);' value="${campaignInfo[0].edDate }"><span
										class="input-group-addon"> <i
										class="glyphicon glyphicon-calendar"></i>
									</span>   
								</div>
						...
						<div class="page7_sub">
							...
							<div class="cam9_bottom">
								...
								<textarea
									...
									class="regi form-control">${campaignInfo[0].cExp }</textarea>
								<div class="document">
									...
									<form class="form1" action="/main/modify/fileUpload.do?_csrf=${_csrf.token }" method="POST" encType="multipart/form-data">
										<c:forEach var="item" items="${campaignFile}" varStatus="status" begin="0"  step="1" end="${fn:length(campaignFile) }">
											<div id="customFile${status.index }">
												<i class="far fa-file-image" style="font-size: 24px"></i> 
												<input type='file' accept='image/jpg,image/png,image/jpeg,image/gif' id='profile_img_upload' name="profile_img_upload${status.index}" onchange="existFileChange(this, ${status.index})"/> 
												<div id="existingFile${status.index}" class="exist" style="width: 165px; overflow: hidden; text-overflow : ellipsis; padding-top: 9px; padding-left: 6px; text-align: left">${item.fileOrgName}</div>
												<c:if test="${status.count ne status.end }"> <i id="plusbt" class="fa-sharp fa-solid fa-plus" style="font-size: 24px; display:none;" onclick="addFile()"></i></c:if> 
												<c:if test="${status.count eq status.end }"> <i id="plusbt" class="fa-sharp fa-solid fa-plus" style="font-size: 24px;" onclick="addFile()"></i></c:if>
												<i class="fa fa-close" style="font-size: 24px" onclick="deleteFile(${status.index})"></i>
												<input type="hidden" name="form1" value="form1">
											</div>
										</c:forEach>
<!-- 										<input class="formSubmit" type="submit" value="submit" onclick="cpn_submit()"> -->
										<c:if test="${campaignFile.size() eq 0}">
											<div id="customFile0">
											<i class="far fa-file-image" style="font-size: 24px"></i> 
							...
							<input class="formSubmit" type="submit" value="캠페인  수정 완료" >
							<!-- <button onclick="cpn_submit()">기존파일 업데이트</button> -->
						</div>
					</div>
				</div>
			</div>
...
<!-- 	<!-- Initialize Swiper --> -->
	<script src="/main/js/main/modify/campaignModify.js"></script>
